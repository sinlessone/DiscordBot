async function fetchUser(client, idOrResolvable) {
    ...
    if (typeof idOrResolvable === 'object') {
        ...
        if (idOrResolvable.id && idOrResolvable.username) return idOrResolvable;
    }
...
async function sendDMWithFallback(client, userResolvable, fallbackChannel, messageOptions) {
    const dmResult = await sendDM(client, userResolvable, messageOptions);
    if (dmResult.success) return { sentTo: 'dm', result: dmResult };

    try {
        if (fallbackChannel && typeof fallbackChannel.reply === 'function') {
            const res = await fallbackChannel.reply(messageOptions);
            return { sentTo: 'fallback', result: { success: true, message: res } };
        }
        ...
        return { sentTo: null, result: { success: false, error: new Error('No valid fallback channel') } };
    } catch (err) {
        return { sentTo: null, result: { success: false, error: err } };
    }
}

async function awaitDMReply(client, userResolvable, promptOptions, timeoutMs = 60000) { // untested i think
    const user = await fetchUser(client, userResolvable);
    if (!user) return null;
    ...
    try {
        ...
        const collected = await dmChannel.awaitMessages({ filter, max: 1, time: timeoutMs, errors: ['time'] }).catch(() => null);
